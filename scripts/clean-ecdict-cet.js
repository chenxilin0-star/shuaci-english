...
function clean() {
  ...
  const payload = { generatedAt: new Date().toISOString(), source: input, wordBanks, words, grammarTopics: [], readingPassages: [] };
  const outPath = path.join(root, output);
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(payload, null, 2), 'utf8');
  console.log(`cleaned ${words.length} CET words -> ${output}`);
}
if (require.main === module) clean();
module.exports = { parseCsv, difficulty, frequencyTag };
