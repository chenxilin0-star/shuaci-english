...
exports.main = async (event) => { const wxContext=cloud.getWXContext(); const data={openid:wxContext.OPENID||event.openid||'mock-openid', date:new Date().toISOString().slice(0,10), checked:true, streakDays:event.streakDays||13}; try { await db.collection('checkins').add({data}); return ok(data); } catch(e) { return ok(data); } };
