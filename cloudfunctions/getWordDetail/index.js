...
exports.main = async (event) => { try { const res = await db.collection('words').where({id:event.wordId}).limit(1).get(); return ok(res.data[0] || mock.words.find(w=>w.id===event.wordId||w.text===event.wordId) || mock.words[0]); } catch(e) { return ok(mock.words.find(w=>w.id===event.wordId||w.text===event.wordId) || mock.words[0]); } };
