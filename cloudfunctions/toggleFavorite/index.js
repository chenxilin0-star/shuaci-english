...
exports.main = async (event) => { const wxContext=cloud.getWXContext(); const data={openid:wxContext.OPENID||event.openid||'mock-openid', itemId:event.itemId, itemType:event.itemType||'word', isFavorite:!!event.isFavorite, updatedAt:new Date()}; try { await db.collection('favorites').add({data}); return ok(data); } catch(e) { return ok(data); } };
