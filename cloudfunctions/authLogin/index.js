...
exports.main = async (event, context) => { const wxContext = cloud.getWXContext(); const openid = wxContext.OPENID || 'mock-openid'; const user = { openid, nickName: event.nickName || '刷词同学', avatarUrl: event.avatarUrl || '', lastLoginAt: new Date() }; try { await db.collection('users').where({openid}).update({data:{lastLoginAt:user.lastLoginAt}}); return ok(user); } catch(e) { return ok(user); } };
