...
exports.main = async (event) => { const wxContext=cloud.getWXContext(); const data={...event, openid:wxContext.OPENID||event.openid||'mock-openid', updatedAt:new Date()}; try { await db.collection('user_progress').add({data}); return ok(data); } catch(e) { return ok(data); } };
